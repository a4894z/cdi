function [ sol, expt ] = ptycho2DTPA_runGPUiterations_v4( sol, expt, N_epochs, N_exwvup )
...
%             [ sol.GPU ] = CPUmem( sol, expt );
    ...
    %         exwv_diff = zeros( 1, N_exwvup, 'single' );
            ...
            for gg = 1 : N_exwvup
                ...
                S2 = num2str( [ sol.it.epoch, uu, sol.spos.batch_N_iteration, gg, N_exwvup ], 'Epoch = %d, Iteration = %d / %d, Update Repeat = %d / %d' );
...
%         export_fig( num2str( sol.it.exwv, 'probe_%d.jpg' ), '-r90.0' )
        ...
        [ phi, ~ ] = enforce_2DTPAsposview( sol.probe.P, sol.sample.TF, sol.sample.vs, sol.spos.rs( round( 0.5 * expt.spos.N ), : ), sol.spos.shifttype );
