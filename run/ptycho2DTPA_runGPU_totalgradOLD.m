...
%     sol.spos.batch_indxsubset = randperm( length( sol.spos.indxsubset ), sol.spos.N );
    sol.spos.batch_indxsubset = 1 : sol.spos.N;
    
    sol.spos.batch_rs         = sol.spos.rs( sol.spos.batch_indxsubset, : );
    sol.spos.frameindx        = get_indices_2Dframes( sol.spos.batch_rs, sol.sample.sz.sz, sol.sample.vs.r, sol.sample.vs.c );
...
%                                                         sol.GPU.Nspos );
            ...
            [ sol.GPU.TFv ] = batchgradupdate_2DTPAsample( sol.GPU.phi,             ...
                                                           sol.GPU.TFv,             ...
                                                           sol.GPU.probe,           ...
                                                           sol.GPU.ind_offset,      ...
                                                           sol.GPU.rc,              ...
...
%             sol.GPU.probe = sol.GPU.probe + ( sum( conj( TFview ) .* sol.GPU.phi - sol.GPU.probe .* abs2_TFview, 4 ) ) ./ ( 1e-7 + sum( abs2_TFview, 4 ) );
            ...
            % THEN GET ANOTHER SET FROM OLD sol.GPU.probe, ONLY ORTHONONALIZED
            % USING THESE TWO "PROJECTIONS", DM/ePIE UPDATE AND MODE ORTHOG UPDATE, USE RAAR?
...
%             for jj = 1 : 5
%                 
% %                 probe_old = sol.GPU.probe;
%                 
%                 sol.GPU.probe = sol.GPU.probe + ( sum( conj( TFview ) .* sol.GPU.phi - sol.GPU.probe .* abs2_TFview, 4 ) ) ./ ( 1e-7 + sum( abs2_TFview, 4 ) );
%                 sol.GPU.probe = orthog_modes_eigendecomp( sol.GPU.probe ); 
%                 
% %                 prb_err( jj ) = norm( probe_old(:) - sol.GPU.probe(:));
%                 
%             end
...
%             probe_P_GD = sol.GPU.probe + ( sum( conj( TFview ) .* sol.GPU.phi - sol.GPU.probe .* abs2_TFview, 4 ) ) ./ ( 1e-7 + sum( abs2_TFview, 4 ) );
%             
%             probe_R_GD = 2 * probe_P_GD - sol.GPU.probe;
%             
%             
%             probe_P_ORTHOG_R_GD = orthog_modes_eigendecomp( probe_R_GD ); 
%             
%             probe_R_ORTHOG_R_GD = 2 * probe_P_ORTHOG_R_GD - probe_R_GD;
%             
%             
%             bbeta = 0.3333;
%             
%             sol.GPU.probe = bbeta * 0.5 * ( probe_R_ORTHOG_R_GD + sol.GPU.probe ) + ( 1 - bbeta ) * probe_P_GD;
