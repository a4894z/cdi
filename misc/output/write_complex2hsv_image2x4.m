function write_complex2hsv_image2x4( p1, p2, p3, p4, p5, p6, p7, p8, wrte_name, out_prob_sz, log_absrptn, globl_phs, brightness_scaling )
...
p7 = imresize2D(p7, [szR, szC]);
p8 = imresize2D(p8, [szR, szC]);
...
p_all( 1 : szR, (szC + 1 + szC  + szC) : (szC + szC + szC + szC) ) = p4;
p_all( szR + 1 : 2 * szR, 1 : szC ) = p5;
p_all( szR + 1 : 2 * szR, (szC + 1) : (szC + szC) ) = p6;
p_all( szR + 1 : 2 * szR, (szC + 1 + szC) : (szC + szC + szC) ) = p7;
p_all( szR + 1 : 2 * szR, (szC + 1 + szC + szC) : (szC + szC + szC + szC) ) = p8;

bullseye_color = pi * 1;
bullseye_bright = 0.8;
p_all( szR/2 + 1, 1 : (szC + szC + szC + szC) ) = bullseye_bright * exp(1i * bullseye_color); 
p_all( szR + szR/2 + 1, 1 : (szC + szC + szC + szC) ) = bullseye_bright * exp(1i * bullseye_color); 
...
p_all( 1 : 2 * szR, 3 * szC + szC/2 + 1 ) = bullseye_bright * exp(1i * bullseye_color);
