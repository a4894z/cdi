function [ A, b ] = paralleltomo_v0( N, theta, p, d, isDisp, isMatrix, sample )
...
%==================================================================================================
    ...
    % Construct either matrix or function handle, either way get sinogram
    if isMatrix

        A = get_or_apply_system_matrix( N, theta, p, d, isDisp, true, [], '' );
        
        b = A * sample;
        
    else

        A = @( U, TRANSP_FLAG ) get_or_apply_system_matrix( N, theta, p, d, isDisp, false, U, TRANSP_FLAG );
...
function A = get_or_apply_system_matrix( N, theta, p, d, isDisp, isMatrix, u, transp_flag )
    ...
    if isMatrix
        ...
        % Initialize vectors that contains the row numbers, the column numbers and the values for creating the matrix A effiecently.
        ...
        % If transp_flag is 'size', only return size of operator, otherwise set proper size of output.
        switch transp_flag
        ...
        % If u is a Cartesian unit vector then we only want to compute a single row of A; otherwise we want to multiply with A or A'.
    ...
    for i = II
        ...
        for j = JJ
            ...
            % Use the parametrisation of line to get the y-coordinates of intersections with x = constant.
            tx = ( x - x0theta( j )) / a;
            yx = b * tx + y0theta( j );

            % Use the parametrisation of line to get the x-coordinates of intersections with y = constant.
            ty = ( y - y0theta( j )) / b;
            xy = a * ty + x0theta( j );
            ...
            if numel( aval ) > 0
                ...
                % If the ray is on the boundary of the box in the top or to the right the ray does not by definition lie with in a valid cell.
                if ~( ( b == 0 && abs( y0theta( j ) - N/2 ) < 1e-15 ) || ( a == 0 && abs( x0theta( j ) - N/2 ) < 1e-15 ) )
