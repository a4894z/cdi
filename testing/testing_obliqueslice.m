...
clear; close all; testing_obliqueslice
...
figure; montage( V, 'Indices', 12:118, 'Size', [ 8, 12 ], 'DisplayRange', [] );
...
for xslice = 10 : 5 : 180
    ...
    [ B(:,:,cnt), x(:,:,cnt), y(:,:,cnt), z(:,:,cnt) ] = obliqueslice_testing( V, point{ cnt }, normal, 'OutputSize', 'Full', 'FillValues', 255 );
...
for slice = 1:size(B,3)
    ...
    zlim([0 155]); ylim([0 250]); xlim([0 250]);
...
function [B, XData, YData, ZData] = obliqueslice_testing(varargin)
...
%   Copyright 2019-2020 The MathWorks, Inc.
    ...
    % point and origin
    D = -1 * ( unitNormalVector( 1 ) * point(1) + unitNormalVector( 2 ) * point( 2 ) + unitNormalVector( 3 ) * point( 3 ));
    ...
    obliqueSlice = interp3( V, xq, yq, zq, method, fillValue );
    
%     % Obtain slice at desired coordinates using interpolation
%     obliqueSlice2 = images.internal.builtins.interp3d_halide( V, xq, yq, zq, method, fillValue );
%     norm( obliqueSlice(:) - obliqueSlice2(:) )
...
function [ V, point, normal, method, outputsize, fillValues, catConverter ] = parse_inputs(varargin)
    ...
    parser.addRequired('point',@(point) validateattributes(point,{'numeric'},{'size',[1,3],'real','finite','nonsparse'}, mfilename,'point',2));

    % Validate normal vector to the plane
    normal = varargin{3};
    parser.addRequired('normal', @(normal) validateattributes(normal,{'numeric'},attributes,mfilename,'normal',3));
    ...
    method = validatestring(parser.Results.Method, {'nearest','linear'}, mfilename, 'Method');
...
function TF = validateFillValues(fillVal)
    ...
    validateattributes( fillVal, {'numeric'}, {'nonempty','nonsparse','scalar','real'}, 'obliqueslice', 'FillValues' );
    TF = true;
